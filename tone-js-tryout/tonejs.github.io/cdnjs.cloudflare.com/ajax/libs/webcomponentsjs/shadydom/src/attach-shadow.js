...
import {
    calculateSplices
} from './array-splice.js';
import * as utils from './utils.js';
import {
    enqueue
} from './flush.js';
import {
    ensureShadyDataForNode,
    shadyDataForNode
} from './shady-data.js';
import {
    recordChildNodes
...
class ShadyRoot {
    ...
    _addAssignedToFlattenedNodes(flattened, assigned) {
        for (let i = 0, n;
            (i < assigned.length) && (n = assigned[i]); i++) {
            if (n.localName == 'slot') {
    ...
    _updateChildNodes(container, children) {
        ...
        // process removals
        for (let i = 0, d = 0, s;
            (i < splices.length) && (s = splices[i]); i++) {
            for (let j = 0, n;
                (j < s.removed.length) && (n = s.removed[j]); j++) {
        ...
        // process adds
        for (let i = 0, s, next;
            (i < splices.length) && (s = splices[i]); i++) { //eslint-disable-line no-redeclare
...
export const attachShadow = (host, options) => {
    ...
    if (options['shadyUpgradeFragment'] && utils.canUpgrade()) {
        ...
        /** @type {ShadowRoot} */
        (root).host[utils.NATIVE_PREFIX + 'appendChild'](root);
    } else {
